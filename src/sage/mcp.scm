...
to avoid hanging on the SSE long-lived connection."
  (catch #t
    (lambda ()
      (let* ((cmd "curl -s --connect-timeout 2 --max-time 3 -o /dev/null -w '%{http_code}' -H 'Authorization: Bearer REDACTED_AUTH_TOKEN' -H 'Accept: text/event-stream' http://INFRA_HOST:8400/sse")
