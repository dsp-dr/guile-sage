...
5. **Telemetry warning visible via read_logs.** A `[WARN] [telemetry] Flush failed (non-2xx) code=0 endpoint=${OTEL_HTTP_ENDPOINT}/v1/metrics` line appeared in the log output — unrelated to qwen3 but worth surfacing: if infra-host is unreachable, every tool turn leaves a warning in the JSONL log, which then gets fed back to the model on the next `read_logs` call.
